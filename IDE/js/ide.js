var apiUrl = localStorageGetItem("api-url") || "http://buildit.iare.ac.in:3000";
var serverUrl = "http://buildit.iare.ac.in:5000";
...
$(document).ready(function () {
    ...
    if (getCookie("courseId")){
        ...
        courseId = getCookie("courseId");
        if (courseId === "IARE_PY"){
            $('#select-language').append(`<option mode = "python" value="34"> Python (3.6.0) </option>`);
        } else if (courseId === "IARE_C"){
            $('#select-language').append(`<option mode = "c" value="4"> C (gcc 7.2.0) </option>`);
        } else if (courseId === "IARE_CPP"){
            $('#select-language').append(`<option mode = "cpp" value="10"> C++ (g++ 7.2.0) </option>`);
        } else if (courseId === "IARE_JAVA"){
            $('#select-language').append(`<option mode = "java" value="26"> Java (OpenJDK 9 with Eclipse OpenJ9) </option>`);
        }
    } else if (getCookie("contestId")) {
        $('#tutorials').text('Contest Ongoing')
        $('#select-language').append(`<option value="1" mode="shell">Bash (4.4)</option>
